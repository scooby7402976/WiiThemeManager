...
static u32 get_sig_len(u8 *sig)
{
	...
	fprintf(stderr, "get_sig_len(): unhandled sig type %08lx\n", type);
	return 0;
}
...
static u32 get_sub_len(u8 *sub)
{
	...
	fprintf(stderr, "get_sub_len(): unhandled sub type %08lx\n", type);
	return 0;
}
...
void dump_tmd(u8 *tmd)
{
	...
	p = tmd + 0x01e4;
	for (i = 0; i < n; i++) {
		printf("cid %08lx  index %04x  type %04x  size %08llx\n",
